...
API routes in `src/app/api/` (`contact`, `chat`, `leads`, `shopify-lead`) validate input with Zod, then **forward to external n8n webhooks** configured via env vars (`N8N_WEBHOOK_URL`, `N8N_CHAT_WEBHOOK_URL`, `N8N_LEAD_WEBHOOK_URL`). Each has a **graceful fallback when the webhook env var is unset** (contact also sends SMTP mail if configured; otherwise logs to console), so the site runs fully in local dev with no env setup. The contact route also does honeypot (`_hp`) filtering and per-IP in-memory rate limiting. See `.env.example` for the full env surface.
